...
namespace mogo
{
    ...
    CWorldLogin::CWorldLogin() : m_bCanLogin(true), m_onlineCount(0)
    {
    }
    ...
    int CWorldLogin::FromRpcCall(CPluto& u)
    {
        ...
        switch(msg_id)
        {
            ...
            {
				LogWarning("FromRpcCall", "msgid:MSGID_LOGINAPP_LOGIN");
                AddClientFdToVObjectList(u.GetMailbox()->GetFd(), p);
                nRet = AccountLogin(p);
                break;
            }
            ...
            {
                LogWarning("FromRpcCall", "msgid:MSGID_LOGINAPP_SELECT_ACCOUNT_CALLBACK");
	...
	{
        LogWarning("FromRpcCall", "msgid:MSGID_LOGINAPP_LOGIN p=%d", p->size());
		if(p->size() != 3)
		{
			return -1;
		}
        ...
        LogInfo("CWorldLogin::account_login", "fd2accounts, accounts2fd insert");
        if(bLoginFirst)
        {
            //开发模式,要先校验一下
            ...
            if(mb2)
            {
                LogInfo("CWorldLogin::account_login", "MSGID_DBMGR_SELECT_ACCOUNT:server_name=%s,server_port=%d", 
                    mb2->GetServerName().c_str(), mb2->GetServerPort());
        ...
        else
        {
            ...
            if(mb2)
            {
                ...
                LogInfo("CWorldLogin::account_login", "MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME_ANYWHERE");
