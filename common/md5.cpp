...
#define II(a, b, c, d, x, s, ac) { \
	...
	const uint8_t CMd5::PADDING[64] = { 0x80 };
	...
	void CMd5::update(ifstream& in) {
		...
		while (!in.eof()) {
			...
			if (length > 0) {
				len += length;
				//char* p = replace(buffer, "\r\n", "\n");
				//char* p2 = replace(p, "\t", "");
				update(buffer, length);
			}

		}
		//LogDebug("CMd5::update(ifstream& in)", "len = %d", len);
		in.close();
	...
	*/
	...
	void CMd5::update(const uint8_t* input, size_t length) {
	...
	*/
	...
	void CMd5::transform(const uint8_t block[64]) {
	...
	*/
	...
	void CMd5::encode(const uint32_t* input, uint8_t* output, size_t length) 
	{
	...
	*/
	...
	void CMd5::decode(const uint8_t* input, uint32_t* output, size_t length) {
	...
	string CMd5::bytesToHexString(const uint8_t* input, size_t length) {
	...
	//
	char* CMd5::replace(char *source, char *sub, char *rep)    
	{    
		char *result;    
		char *pc1, *pc2, *pc3;                    
		...
		if (isub < irep)
		{
			LogError("CMd5::replace", "");
			return source;
		}
		isource = strlen(source);  
		if('\0' == *sub)        
			return source; /*strdup(source);  */  
		/*ÉêÇë½á¹û´®ÐèÒªµÄ¿Õ¼ä*/
